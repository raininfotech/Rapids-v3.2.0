...
const char * const Rapids_CONF_FILENAME = "rapids.conf";
const char * const Rapids_PID_FILENAME = "rapids.pid";
const char * const Rapids_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RapidsParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RapidsParams
    // Mac: ~/Library/Application Support/RapidsParams
    // Unix: ~/.rapids-params
#ifdef WIN32
    // Windows
...
{
    fs::path pathConfigFile(GetArg("-conf", Rapids_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", Rapids_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", Rapids_PID_FILENAME));
