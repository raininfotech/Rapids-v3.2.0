...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    entry.push_back(Pair("rpdconfirmations", confirms));
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"rapidsaddress\"   (string, required) The rapids address to be associated with a label.\n"
...
struct tallyitem {
    ...
    tallyitem()
    {
        ...
        nRPDConf = std::numeric_limits<int>::max();
        fIsWatchonly = false;
    }
};
...
UniValue ListReceived(const UniValue& params, bool by_label)
{
    ...
    for (std::map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        ...
        int nRPDDepth = wtx.GetDepthInMainChain(false);
        if (nDepth < nMinDepth)
            continue;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            item.nRPDConf = std::min(item.nRPDConf, nRPDDepth);
    ...
    for (auto item_it = start; item_it != end; ++item_it)
    {
        ...
        int nRPDConf = std::numeric_limits<int>::max();
        bool fIsWatchonly = false;
        if (it != mapTally.end()) {
            nAmount = (*it).second.nAmount;
            nConf = (*it).second.nConf;
            nRPDConf = (*it).second.nRPDConf;
        ...
        if (by_label) {
            ...
            _item.nRPDConf = std::min(_item.nRPDConf, nRPDConf);
            ...
            obj.push_back(Pair("rpdconfirmations", (nRPDConf == std::numeric_limits<int>::max() ? 0 : nRPDConf)));
    ...
    if (by_label) {
        for (const auto& entry : label_tally) {
            ...
            int nRPDConf = entry.second.nRPDConf;
            UniValue obj(UniValue::VOBJ);
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"rpdconfirmations\" : n              (numeric) The number of blockchain confirmations of the most recent transaction included\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions ( \"dummy\" count from includeWatchonly includeDelegated )\n"
            ...
            "    \"rpdconfirmations\": n,     (numeric) The number of blockchain confirmations for the transaction. Available for 'send'\n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "    \"rpdconfirmations\" : n,    (numeric) The number of blockchain confirmations for the transaction. Available for 'send' and 'receive' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"rpdconfirmations\" : n,   (numeric) The number of blockchain confirmations\n"
