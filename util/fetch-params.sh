...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/RapidsParams"
    else
        PARAMS_DIR="$HOME/.rapids-params"
    fi
fi
...
Failed to fetch the Rapids zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Rapids - fetch-params.sh

This script will fetch the Rapids zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Rapids zkSNARK parameters. Note that it is
